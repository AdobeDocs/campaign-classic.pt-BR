...
* Correção de uma falha intermitente com descriptografia GPG nas atividades de fluxo de trabalho de **Script**. A mensagem de erro associada era: `gpg: decryption failed: No secret key`. (NEO-50257)  <!--* Workflow temporary tables now have a primary index in Teradata with a Federated Data Access (FDA) connection. (NEO-62575)-->
