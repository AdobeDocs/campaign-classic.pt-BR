...
1. Desligue os seguintes serviços:
   ...
   * Serviço Adobe Campaign: **net stop nlserver6**
   >[!IMPORTANT]
   >
   >Você também precisa verificar se o servidor de redirecionamento (webmdl) está parado, para que a variável **nlsrvmod.dll** O arquivo usado pelo IIS pode ser substituído pela nova versão.

1. Verifique se nenhuma tarefa está ativa executando o **pdump nlserver** comando. Deve aparecer o seguinte:
...
1. Selecione o modo de instalação: Choose **[!UICONTROL Update or repair]**
...
>[!NOTE]
>
>Esta operação deve ser executada somente uma vez e somente em um (**nlserver web**) servidor de aplicativos.
...
* O **postupgrade_`<server version number>_<time of postupgrade>`.log** o arquivo de log contém o resultado da sincronização. Está disponível por padrão no seguinte diretório: **`<installation directory>/var/<instance/postupgrade`**. Erros e avisos são indicados pelos atributos de erro e aviso.
...
1. Na seção inferior da janela, procure pelo **_conflito_** sequência de caracteres para localizar as entidades com conflitos. A entidade instalada com a nova versão contém a variável **novo** , a entidade que corresponde à versão anterior contém a variável **cus** argumento .
...
1. Vá para o conflito que você resolveu. Clique no botão **[!UICONTROL Actions]** e selecione **[!UICONTROL Declare as resolved]** .
...
Na máquina em que o servidor de aplicativos do Adobe Campaign está instalado (**nlserver web**), baixar e copiar o arquivo  **setup-client-6.XXXX.exe** i n **[caminho do aplicativo]/datakit/nl/eng/jsp**.
...
Na máquina em que o servidor de aplicativos do Adobe Campaign (**nlserver web**) estiver instalado, recupere o  **setup-client-6.XXXX.exe** e copie-o, salvando como **/usr/local/neolane/nl6/datakit/nl/eng/jsp**:
...
>[!NOTE]
>
>Verifique se o usuário do Apache tem os direitos de leitura apropriados para esse arquivo de instalação e consulte [guia de instalação](../../installation/using/general-architecture.md) para obter mais informações.
