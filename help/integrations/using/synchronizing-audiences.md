---
product: campaign
title: Sincronizar públicos-alvos
description: Saiba como sincronizar públicos-alvos com o Conector ACS
...
# Sincronizar públicos-alvos{#synchronizing-audiences}



Você pode criar uma lista sofisticada usando os recursos avançados do Campaign v7 e compartilhar essa lista como um público-alvo diretamente e em tempo real com o Campaign Standard (incluindo dados adicionais) de forma contínua. O usuário do Campaign Standard pode então consumir o público-alvo no Adobe Campaign Standard.

Direcionamentos complexos envolvendo dados adicionais que não são replicados no Campaign Standard só podem ser alcançados usando o Campaign v7.
...
* Dados adicionais, como assinaturas ou transações armazenadas em tabelas relacionadas a nms:recipients no banco de dados do Campaign v7. Esses dados podem ser dos esquemas de OOB ou em tabelas personalizadas do Campaign v7. Por padrão, eles não estão disponíveis no Campaign Standard, pois não são sincronizados.
* Direito para executar fluxos de trabalho no Campaign v7 e Campaign Standard.
...
>[!NOTE]
>
>Este é um exemplo. Dependendo dos requisitos, você pode simplesmente fazer uma consulta de uma lista de destinatários e compartilhá-la com ACS sem nenhum processamento adicional. Você também pode usar outras atividades de gestão de dados para preparar seu target final.

Para obter o público-alvo final e seus dados adicionais:

1. Crie um novo fluxo de trabalho a partir de **[!UICONTROL Profiles and Targets]** > **[!UICONTROL Jobs]** > **[!UICONTROL Targeting workflows]**.
...
1. Adicionar dados adicionais dentro da consulta. Para obter mais informações, consulte a seção [Adição de dados](../../workflow/using/query.md#adding-data).
...
1. Salve o fluxo de trabalho. A próxima seção demonstra como compartilhar a população com o ACS.

## Compartilhamento do target com o Campaign Standard {#share-the-target-with-campaign-standard}

Uma vez definida a população de destino, é possível compartilhá-la com o ACS por meio de uma atividade **[!UICONTROL List update]**.

1. No fluxo de trabalho criado anteriormente, adicione uma atividade **[!UICONTROL List update]** e especifique a lista que deseja atualizar ou criar.
...
1. Salve e execute o fluxo de trabalho.

   O target e seus dados adicionais são salvos em uma lista no Campaign v7 e compartilhados imediatamente como uma lista de público-alvo no Campaign Standard. Somente os perfis replicados são compartilhados com ACS.
...
Depois que o fluxo de trabalho de direcionamento é executado no Campaign v7, é possível encontrar o público-alvo da lista no modo somente leitura do menu **[!UICONTROL Audiences]** no Campaign Standard.

![](assets/acs_connect_deliveryworkflow_audience.png)

Ao criar um workflow de entrega no Campaign Standard, é possível usar esse público-alvo e os dados adicionais contidos em uma entrega.

1. Crie um novo fluxo de trabalho pelo menu **[!UICONTROL Marketing activities]**.
1. Adicione uma atividade **[!UICONTROL Read audience]** e selecione o público-alvo compartilhado anteriormente pelo Campaign v7.

   Essa atividade é usada para recuperar os dados do público-alvo selecionado. Se necessário, também é possível aplicar um **[!UICONTROL Source Filtering]** adicional, usando a guia relativa a esta atividade.
...
1. Insira um campo de personalização Localize o nó **[!UICONTROL Additional data (targetData)]** na janela popup. Esse nó contém os dados adicionais do público-alvo calculados no workflow inicial para construção do target. Você pode usá-los como qualquer outro campo de personalização.

   Para este exemplo, os dados adicionais provenientes do fluxo de trabalho original de segmentação são o número de entregas enviadas a cada destinatário nos últimos 365 dias. O alias NBdeliveries especificado no fluxo de trabalho de segmentação está visível aqui.

   ![](assets/acs_connect_deliveryworkflow_targetdata.png)

1. Salve a entrega e o fluxo de trabalho.

   O fluxo de trabalho agora está pronto para ser executado. A entrega será analisada e estará pronta para ser enviada.
...
1. Execute o fluxo de trabalho de entrega. Essa etapa prepara o email para envio.
...
1. Monitore relatórios e logs de entrega:
   ...
   * **no Campaign v7 e no Campaign Standard**: IDs de entrega, logs amplos de email e de rastreamento de email são sincronizados com o Campaign v7. Você pode obter a visão de 360° das campanhas de marketing do Campaign v7.
     ...
     As quarentenas são sincronizados automaticamente de volta ao Campaign v7. Isso possibilita que as informações não entregues sejam consideradas para o próximo direcionamento realizado no Campaign v7.
