...
Com o Campaign Classic, todos os usuários regulares já podem se conectar ao console do cliente do Adobe Campaign usando sua Adobe ID, por meio do Adobe Identity Management System (IMS). No entanto, as conexões de usuário/senha ainda estão disponíveis. Isso não será mais permitido com o Campaign v8.
...
A Adobe apoia você nesse esforço de migração. Você pode encontrar contexto detalhado e diretrizes passo a passo no artigo abaixo.
...
Antes de iniciar o processo de migração, entre em contato com o Gerenciador de transição do Adobe (para clientes do Managed Services) ou com o Atendimento ao cliente da Adobe (para outros clientes hospedados), para que as equipes técnicas da Adobe possam migrar seus grupos de operadores e direitos nomeados existentes para o Adobe Identity Management System (IMS).
...
1. O administrador interno do Campaign deve adicionar emails exclusivos a todos os usuários nativos no console do cliente do Campaign e confirmar para o representante da Adobe/Atendimento ao cliente depois que isso for concluído.  Esta etapa está detalhada em [esta seção](#ims-migration-id).
1. Entre em contato com o representante da Adobe/Atendimento ao cliente para garantir uma data na qual a Adobe executará a migração automatizada para seus usuários (operadores) e perfis de produto não técnicos. Esta etapa requer uma janela de uma hora sem tempo de inatividade para nenhum de seus serviços.
...
Depois que essa migração for concluída, confirme para o Gerenciador de transição do Adobe (para usuários do Managed Services) ou para o Atendimento ao cliente da Adobe (para clientes hospedados). O Adobe marca a migração como concluída. Seu ambiente é então protegido e padronizado.
...
A Adobe recomenda criar somente usuários do IMS após atualizar para o Campaign Classic v7.4.1 (ou uma [versão compatível com a migração do IMS](#ims-versions)).
...
Para clientes do Hosted e do Managed Services, para finalizar a migração (migrar usuários e perfis de produtos), o Adobe requer um período de uma hora sem tempo de inatividade para nenhuma de suas instâncias (workflows etc.).
