...
## Sobre fluxos de trabalho técnicos {#overview}
...
Para obter mais informações sobre como monitorar fluxos de trabalho técnicos, consulte a [seção específica](monitoring-technical-workflows.md).

## Lista de fluxos de trabalho técnicos {#list-technical-workflows}

| Fluxo de trabalho técnico | Pacote | Descrição |
|------|--------|-----------|
| **Limpeza de alias** (aliasCleansing) | Entrega | Esse fluxo de trabalho padroniza os valores de enumeração. É acionado todos os dias às 3h por padrão. |
...
| **Cálculo de estatísticas do Twitter** (statsTwitter) | Redes sociais (Marketing Social) – Somente Campaign v7 | Este fluxo de trabalho calcula estatísticas vinculadas a retweets e visitas ao X (anteriormente conhecido como Twitter). |
| **Processos do Campaign** (operationMgt) | Campanhas de marketing (Campaign) | Esse fluxo de trabalho gerencia os processos das campanhas de marketing (inicia o direcionamento, faz a extração de arquivos etc.). Ele também cria fluxos de trabalho relacionados a campanhas recorrentes e periódicas. |
...
| **Processos de fórum de discussão** (newsgroupMgt) | Recursos de marketing (MRM) | Este fluxo de trabalho gerencia a entrega de notificações dos fóruns de discussão. É acionado ao receber um sinal de aprovação |
...
| **Mid-sourcing (contadores de entrega)** (defaultMidSourcingDlv) | Transferência para mid-sourcing | Esse fluxo de trabalho coleta informações de contagem para entregas no servidor mid-sourcing. Informações de contagem incluem indicadores de entrega gerais, como número de entregas enviadas etc. Informações de rastreamento, como aberturas, não estão incluídas. É acionado a cada dez minutos por padrão. |
...
| **Limpeza de fluxos de trabalho pausados** (cleanupPausedWorkflows) | Entrega | Triggers is translated correctly in this case. Esse fluxo de trabalho analisa fluxos de trabalho pausados que têm a severidade definida como normal e emite avisos e notificações quando ficam pausados por muito tempo. Após um mês, os fluxos de trabalho técnicos pausados são interrompidos definitivamente. Por padrão, é acionado toda segunda-feira às 5h. Para obter mais informações, consulte[Tratamento de fluxos de trabalho pausados](monitoring-workflow-execution.md#handling-of-paused-workflows). |
...
| **Sincronização de páginas do Twitter** (syncTwitter) | Redes sociais (Marketing Social) – Somente Campaign v7 | Este fluxo de trabalho importa seguidores do X para o Adobe Campaign todos os dias às 7h. |
