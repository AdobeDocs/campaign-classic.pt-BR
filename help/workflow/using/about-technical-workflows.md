...
| **Atualizar para entrega** (material de entregaUpdate) | Monitoramento da entregabilidade (Entregabilidade por email) | Depois que o pacote de monitoramento de entrega (entrega por email) for instalado, esse fluxo de trabalho será executado noturna para atualizar regularmente a lista de regras e permitir o gerenciamento ativo da entrega da plataforma. |
...
| **Atualização para o material de entrega** (material de entregaUpdate) | Delivery | Esse workflow permite criar a lista de regras de qualificação de email de devolução, bem como a lista de domínios e MXs na plataforma. Este workflow funciona somente se a porta HTTPS estiver aberta. Essas listas não são atualizadas a menos que o módulo Deliverability esteja instalado. |
