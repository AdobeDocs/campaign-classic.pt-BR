...
A atividade **[!UICONTROL Load (File)]** permite acessar diretamente uma fonte de dados externos e usá-la no Adobe Campaign. De fato, todos os dados necessários para operações de target nem sempre são encontrados no banco de dados do Adobe Campaign: ele pode ser disponibilizado em arquivos externos.
...
* **[!UICONTROL Error processings]**: define o comportamento se um erro for encontrado.
   ...
   * **[!UICONTROL Reject the line when there is no remapping value]**: a linha inteira só será processada se um mapping tiver sido definido para o valor errado (consulte a opção **[!UICONTROL Mapping]** abaixo).
   * **[!UICONTROL Use a default value in case the value is not remapped]**: substitui o valor que causa o erro por um valor padrão, definido no campo **[!UICONTROL Default value]**, a menos que um mapping tenha sido definido para o valor incorreto (consulte a opção **[!UICONTROL Mapping]** abaixo).
...
1. O coletor de arquivos permite recuperar arquivos armazenados em um diretório, dependendo da frequência fornecida.
   ...
   Para fazer isso, clique no botão **[!UICONTROL Change...]** localizado na seção inferior direita da ferramenta de edição e configure o agendamento.
...
1. Então especifique os processos a serem executados para cada tipo de público. No nosso exemplo, **[!UICONTROL atualizaremos os dados]** no banco de dados. Para fazer isso, posicione uma atividade **[!UICONTROL Update data]** no final de cada transição de saída da atividade Split.
