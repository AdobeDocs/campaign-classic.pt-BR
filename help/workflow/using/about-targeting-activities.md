...
* **Consulta**: permite executar uma consulta. Consulte [Criação de uma consulta](query.md#creating-a-query).
...
* **União**: permite agrupar o resultado de várias atividades em um único target. Consulte a seção [União](union.md).
* **Intersecção**: permite extrair apenas a população com os mesmos resultados de atividade de entrada. Consulte a seção [Intersectio](intersection.md).
...
* **Ofertas por célula**: permite vincular diferentes ofertas a cada subconjunto de uma população. Consulte a seção [Offers by cell](offers-by-cell.md).
...
* **Enriquecimento**: permite adicionar colunas a uma tabela de trabalho ou fluxo de trabalho. Consulte a seção [Enrichment](../../workflow/using/enrichment.md).
* **Editar esquema**: permite transformar, padronizar e, se necessário, enriquecer dados. Para obter mais informações, consulte a seção [Edit schema](../../workflow/using/edit-schema.md).
* **Motor de oferta**: permite chamar a interação do motor de oferta em um fluxo de trabalho. Consulte a seção [Offer engine](../../workflow/using/offer-engine.md).
* **Eliminação de duplicação**: permite remover duplicados de atividades de entrada. Consulte a seção [Deduplication](../../workflow/using/deduplication.md).
* **Alterar dimensão**: permite alterar a dimensão de segmentação durante o ciclo de construção do fluxo de trabalho Consulte a seção [Change dimension](../../workflow/using/change-dimension.md).
...
* **[!UICONTROL Change data source]**: permite alterar a fonte de dados de um fluxo de trabalho **[!UICONTROL Working table]**. Isso oferece mais flexibilidade para gerenciar dados em diferentes fontes de dados, como FDA, FFDA e o banco de dados local. Consulte a seção [CRM Connector](../../workflow/using/change-data-source.md).
