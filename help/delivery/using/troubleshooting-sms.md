...
Consulte a seção [Informações em cada tipo de PDU](../../delivery/using/sms-protocol.md#information-pdu) acima para saber qual campo está disponível em `BIND PDU`, normalmente você adiciona o código curto em `address_range`, mas isso requer suporte especial do provedor. Entre em contato com eles para saber como eles esperam rotear vários códigos curtos independentemente.
...
## Problema com mid-sourcing (hospedado){#issue-mid-sourcing}

* Se o problema ocorrer em um ambiente mid-sourcing, verifique se o delivery e os logs amplos foram criados e atualizados corretamente no servidor mid-sourcing. Se esse não for o caso, isso não é uma questão de SMS.

* Se tudo funcionar no servidor intermediário e o SMS for enviado corretamente, mas a instância de marketing não for atualizada corretamente, você pode ter um problema de sincronização intermediária.
...
* Se a conexão for bem-sucedida, mas instável, verifique a seção [Problema com conexões instáveis](../../delivery/using/troubleshooting-sms.md#issues-unstable-connection).
...
* A conexão dura menos de 1 hora. As conexões do transmissor Adobe Campaign Classic são uma exceção devido ao modo como o Adobe Campaign Classic MTA funciona.
...
* Se o provedor fechar as conexões enviando um `TCP FIN` ou um `TCP RST packet`, pergunte ao provedor mais informações.
...
* Verifique se a conexão está estável. Uma conexão SMPP deve permanecer ativa por pelo menos 1 hora continuamente, exceto para transmissores no Adobe Campaign Classic. Consulte a seção [Problema com conexões instáveis](../../delivery/using/sms-protocol.md#issues-unstable-connection).

* Se reiniciar o MTA fizer com que o envio do MT funcione novamente por um pequeno período de tempo, provavelmente você terá uma limitação devido a uma conexão instável. Consulte a seção [Problema com conexões instáveis](../../delivery/using/troubleshooting-sms.md#issues-unstable-connection).
...
* Se vir muitos `BIND/UNBIND`, você tem uma conexão instável. Consulte a seção [Problema com conexões instáveis](../../delivery/using/troubleshooting-sms.md#issues-unstable-connection) para obter soluções antes de tentar resolver problemas de mensagens de duplicado.
...
* Verifique se a Adobe Campaign responde com êxito `DELIVER_SM_RESP PDU` em tempo hábil. No Adobe Campaign Classic, isso garante que o SR tenha sido inserido na tabela `providerMsgId` para processamento diferido pelo processo SMS.
...
Se `DELIVER_SM PDU` tiver sido reconhecido pelo conector SMPP estendido da Adobe Campaign Classic, mas o wideLog não for atualizado corretamente, verifique o processo de reconciliação da ID descrito na seção [MT, SR e entradas de banda larga](../../delivery/using/sms-protocol.md#matching-mt) correspondentes.

Se você corrigiu tudo, mas alguns SR inválidos ainda estão nos buffers do provedor, é possível ignorá-los usando a opção &quot;Contagem de confirmação de ID inválida&quot;. Isso deve ser usado com cuidado e redefinido para 0 o mais rápido possível depois que os buffers estiverem limpos.
...
* Verifique se o formato do número de telefone é exatamente o mesmo na tabela quarentena e no registro de delivery. Caso contrário, consulte esta [seção](../../delivery/using/sms-protocol.md#automatic-reply) se tiver problemas com o prefixo plus do formato de número de telefone internacional.
...
Em todas as outras situações, tente analisar as mensagens SMPP detalhadas primeiro e solicitar uma captura de rede somente se houver informações ausentes nos registros detalhados.
...
1. Aguarde 10 minutos para permitir que o servidor recarregue o conta externa.
