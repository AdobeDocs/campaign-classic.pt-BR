...
Por isso, &quot;cotas cumpridas&quot; não é apenas uma questão de configuração, mas também pode estar ligada à reputação. É importante analisar as mensagens de erro no log SMTP.
...
* Métricas de correspondência ou campanha: as taxas de rejeição/rejeição de cancelamento de inscrição/abuso são mais altas do que o normal.
* atividade do assinante: abrir/clicar/transações são menores do que o normal.
...
As reclamações são definidas por assinantes que acessam o botão &quot;isso é spam&quot;. Se o problema com seu delivery foi causado por reclamações, você precisa tentar determinar por que os recipient estão reclamando. Os clientes com altos níveis de reclamação também podem desejar considerar mover seu link de cancelamento de assinatura para a parte superior do email para incentivar os assinantes que decidiram apertar o botão de spam a cancelar a inscrição em vez de reclamar.

Os remetentes podem obter uma grande quantidade de informação a partir das suas queixas de feedback. É importante agrupar os dados e procurar padrões em coisas como origem de aceitação, quanto tempo o endereço foi assinado ou até mesmo determinados dados demográficos comportamentais. As queixas podem frequentemente identificar uma fonte de dados ou segmento com risco dentro do arquivo. O risco é definido como o mais provável de se queixar, o que pode prejudicar a reputação e, por sua vez, as taxas da caixa de entrada.

As queixas também têm origem em assinantes que simplesmente não querem mais receber e-mail. Isso pode ser devido a mensagens excessivas, a percepção que eles têm da mensagem, que eles não estavam esperando a mensagem, ou não se lembram de opt in. Também é importante executar uma auditoria para garantir que todos os pontos de coleta estejam claros e que não haja caixas pré-marcadas em seus pontos de aquisição. Você também deve enviar um email de boas-vindas quando os assinantes optarem por definir o tom e explicar com que frequência eles podem receber emails de você.

**Validade dos dados**

Rejeições graves ocorrem quando você envia para um endereço que não pode ser entregue em um ISP. Um endereço pode não ser entregue por vários motivos, como um endereço com erro ortográfico, uma lista ou fonte de dados incorreta ou o envio para um endereço que estava ativo de uma vez, mas que foi fechado ou encerrado após um período de inatividade. Se você encontrar uma alta rejeição, é importante revisar a lista. Se for de uma nova fonte, analise como os endereços foram coletados e verifique se havia permissão. Dados incorretos também podem vir de endereços com erros ortográficos. Isso pode ser resolvido com um serviço de validação de dados em tempo real ou exigindo uma aceitação confirmada antes de enviar emails de marketing para esse endereço.

**Envolvimento**

Além das reclamações e da validade dos dados, os provedores de internet estão se concentrando mais do que nunca em um compromisso positivo para tomar decisões delivery. Eles procuram saber se seus assinantes estão abrindo seus emails ou excluindo-os sem lê-los. Como eles não compartilham esses dados com os remetentes, devemos utilizar as informações que temos disponíveis e traduzir abrir/clicar/transações como envolvimento.

Como parte da manutenção contínua de reputação, é importante entender como os assinantes estão envolvidos em sua lista e desenvolver uma hierarquia de risco de recenticidade para os assinantes em cada arquivo. A Idade é definida como a última data de abertura/clique/transação ou de inscrição. Esse período pode diferir pela vertical. Determine os segmentos ativos (&quot;seguros&quot;) para cada vertical. Normalmente, são assinantes que estiveram ativos nos últimos 3 a 6 meses.

Reduza a frequência para inativos. Crie uma série de reenvolvimento para iniciativas de risco moderado. Normalmente, são de 6 a 9 meses sem envolvimento. Desenvolver uma campanha de confirmação para inativas de maior risco. Normalmente, são assinantes que não se envolvem em um email há 9 a 12 meses. Por fim, é necessário definir uma regra suspensa e remover assinantes que não tenham sido abertos em &quot;x&quot; meses. Normalmente, recomendamos mais de 12 meses, mas isso pode ser diferente com base no ciclo de vendas e compras.
