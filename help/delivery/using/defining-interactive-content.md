---
product: campaign
title: Definir conteúdo interativo no Adobe Campaign Classic
...
* Enviar emails do AMP para endereços do Gmail, Outlook ou Mail.ru depois de se registrar nos provedores correspondentes.

Para obter mais informações sobre teste e envio de emails do AMP, consulte [esta seção](#targeting-amp-email).

Esse recurso está disponível por meio de um pacote dedicado no Adobe Campaign. Dependendo das suas permissões e do modelo de implantação, é possível instalar esse pacote ou entrar em contato com o Adobe para instalá-lo.

>[!NOTE]
>
> Para arquiteturas híbridas e hospedadas, o pacote deve ser instalado em todos os servidores, incluindo o [servidor mid-sourcing](../../installation/using/mid-sourcing-server.md) e [instância de execução](../../message-center/using/configuring-instances.md#execution-instance).


## Sobre o AMP for Email {#about-amp-for-email}

Use o **AMP for Email** novo formato para incluir componentes AMP em suas mensagens e melhorar a experiência de email com um conteúdo rico e acionável. Com a funcionalidade moderna do aplicativo disponível diretamente nos emails, os recipients podem interagir dinamicamente com o conteúdo na própria mensagem.
...
1. Ao definir seu direcionamento, selecione os recipients que podem exibir o formato AMP. Consulte [Direcionamento de um email do AMP](#targeting-amp-email).
   ...
   >[!NOTE]
   >
   >Atualmente, você só pode enviar emails do AMP para o [endereços de email específicos](#testing-amp-delivery-for-selected-addresses) (para fins de ensaio) ou após [registro](#delivering-amp-emails-by-registering) com os clientes de email aceitos.

1. Envie o email como faria normalmente. Consulte [Envio de um email do AMP](#sending-amp-email).

## Criar conteúdo de email do AMP no Adobe Campaign {#build-amp-email-content}
...
1. Ao criar o delivery de email, selecione qualquer template.
   ...
   >[!NOTE]
   >
   >Um template AMP específico contém um exemplo dos principais recursos que podem ser usados: lista de produtos, carrossel, aceitação dupla, pesquisa e solicitação de servidor avançado.
...
1. Depois de concluir a edição, selecione todo o conteúdo do AMP e copie/cole no [AMP web-based validator](https://validator.ampproject.org) ou em um site semelhante.
   ...
   >[!NOTE]
   >
   >O editor do Adobe Campaign AMP não foi projetado para validação de conteúdo. Use um site externo, como o [Validador baseado na Web do AMP](https://validator.ampproject.org) para verificar se o conteúdo está correto.

1. Faça as alterações necessárias até que o conteúdo do AMP passe na validação.

   ![](assets/amp_validator_pass.png)

1. Para visualizar seu conteúdo, copie e cole o conteúdo validado em [Reprodução do AMP](https://playground.amp.dev) ou um site semelhante.
...
Atualmente, três provedores de email são compatíveis com o teste deste formato: Gmail, Outlook e Mail.ru.

Todas as etapas e especificações necessárias para testar o delivery com o formato AMP em contas Gmail estão detalhadas no [Gmail](https://developers.google.com/gmail/ampemail?), [Outlook ](https://docs.microsoft.com/en-gb/outlook/amphtml/)e [Mail.ru](https://postmaster.mail.ru/amp) documentações do desenvolvedor.

Em particular, os seguintes requisitos devem ser atendidos:
* Siga os requisitos específicos de segurança da AMP para [Gmail](https://developers.google.com/gmail/ampemail/security-requirements), [Outlook](https://docs.microsoft.com/en-gb/outlook/amphtml/security-requirements)e [Mail.ru](https://postmaster.mail.ru/amp/?lang=pt-BR#howto).
...
## Direcionar um email do AMP {#targeting-amp-email}
...
## Enviar um email do AMP {#sending-amp-email}
...
Atualmente, somente o Gmail, Outlook e Mail.ru são compatíveis com o formato AMP, sob determinadas condições. É possível direcionar os endereços de outros provedores de email, mas eles receberão a versão HTML ou texto simples do email.
