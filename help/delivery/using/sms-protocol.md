...
>[!NOTE]
>
>O **protocolo e as configurações do conector SMS** para Adobe Campaign Standard podem ser encontrados nesta [página](https://experienceleague.adobe.com/docs/campaign-standard/using/administrating/configuring-sms/sms-protocol.html#administrating).
>
...
Este documento o guiará pela configuração de conexão entre a Adobe Campaign e um provedor SMPP.
...
* O texto, que é limitado a 140 bytes, o que significa entre 70 e 160 caracteres, dependendo da codificação. Consulte [Codificação de texto SMS](../../delivery/using/sms-protocol.md#sms-text-encoding) abaixo para obter detalhes e limitações.
...
A Adobe Campaign Classic suporta o protocolo SMPP versão 3.4. Este é um protocolo generalizado que permite enviar SMS para um provedor (SMSC) e receber SMS, bem como recibos. Para obter mais informações, consulte a [documentação SMPP](https://smpp.org/SMPP_v3_4_Issue1_2.pdf).
...
>[!NOTE]
>
> O Adobe Campaign Classic suporta apenas o modo TX+RX. Essa limitação se deve à sua arquitetura técnica.
...
Esta PDU deve ser enviada pelo sistema antes de desconectar. Ele deve aguardar a PDU `UNBIND_RESP` correspondente antes de fechar a conexão.
...
Campos notáveis em uma PDU `SUBMIT_SM`:
...
* **short_message**: texto da mensagem. Para SR, contém os dados descritos no apêndice B da especificação do protocolo SMPP. Consulte [Gerenciamento de erros do SR](../../delivery/using/sms-protocol.md#sr-error-management) para obter mais detalhes.

A Adobe Campaign pode ler a ID da mensagem no campo opcional `receipted_message_id` com alguma configuração.

#### DELIVER_SM_RESP {#deliver-sm-resp}

Esta PDU é enviada pela Adobe Campaign para reconhecer o SR e o MO.

A Adobe Campaign Classic reconhece o SR e o MO depois de inseridos no banco de dados. Alguns erros de processamento podem ocorrer mesmo se uma PDU `DELIVER_SM_RESP` bem-sucedida tiver sido enviada. Essa limitação é causada pela arquitetura de software da Adobe Campaign Classic.
...
O SMS multiparte, ou SMS longo, são SMSs enviados em várias partes. Devido a limitações técnicas no protocolo de rede móvel, um SMS não pode ser maior que 140 bytes ou precisará ser dividido. Consulte a seção [codificação de texto SMS](../../delivery/using/sms-protocol.md#sms-text-encoding) para saber mais sobre o número de caracteres que podem se ajustar a um SMS.
...
Uma recomendação é feita na seção Apêndice B da [especificação do protocolo SMPP](https://smpp.org/SMPP_v3_4_Issue1_2.pdf) (página 167), mas isso não lista os códigos de erro reais nem o seu significado.
...
Os conectores Adobe Campaign Classic (exceto SMPP estendido) usam um comportamento codificado que depende do provedor selecionado. O SMPP genérico só distingue entre sucesso e erro, sem detalhes. O registro de delivery pode conter algumas informações que não são garantidas.
...
### Exemplo de processamento SR em SMPP genérico Adobe Campaign Classic Extended {#sr-processing}
...
Você deve **sempre entrar em contato com o provedor SMSC em caso de problemas de codificação**. Apenas os fornecedores do SMSC têm conhecimento da codificação que suportam e das regras especiais que podem aplicar-se devido a limitações na sua plataforma técnica.
...
* Certifique-se de saber quais caracteres pertencem a qual codificação. O GSM7 não suporta totalmente marcas diacríticas ou acentos. Especialmente em francês, onde é e è fazem parte do GSM7, mas ê, â ou ï não fazem parte. O mesmo se aplica ao espanhol.
...
Esta opção permite um controle mais fino sobre o número de conexões, consulte [Conexões simultâneas](../../delivery/using/sms-protocol.md#connection-settings).
...
Define o nome da implementação SMSC. Ele deve ser definido com o nome do seu provedor. Entre em contato com o administrador ou a equipe de entrega para saber o que adicionar neste campo. A função deste campo está descrita na seção [SR error management](../../delivery/using/sms-protocol.md#sr-error-management).
...
#### Número de conexões filho MTA {#number-mta-child}

No Adobe Campaign Classic, define o número de conexões por filho MTA.

O conector Adobe Campaign Classic Extended SMPP pode controlar o número de conexões por filho MTA. Para controlar o limite global de conexões, você terá que limitar o número de processos filhos MTA, o que geralmente significa ter uma plataforma mid-sourcing dedicada para SMS.

Para Adobe Campaign Classic, pode haver um número diferente de conexões de receptor e transmissor:

* **Conexões de transmissor = Número de conexões-filho MTA * número de processos-filho MTA * número de MTAs (se a resposta automática estiver definida) + Número de conexões-filho MTA**

Como sugerido acima, o processo do Adobe Campaign Classic SMS abre mais conexões do transmissor se a resposta automática estiver ativada. Essas conexões extras são usadas para enviar as respostas automáticas.

* **Conexões do receptor = Número de conexões filho MTA**

Se você configurar respostas automáticas, o processo SMS abrirá pares de transmissores/receptores, aumentando o número de conexões de transmissores. Se você não configurou nenhuma resposta automática, ela abrirá somente as conexões do receptor.
...
No Adobe Campaign Classic, a saída de log está no log MTA para tráfego relacionado ao MT e no log SMS para tráfego relacionado ao MO e ao SR.
...
Consulte [Defina um mapeamento específico das codificações definindo](../../delivery/using/sms-protocol.md#SMSC-specifics) para obter uma explicação mais geral do processo de codificação.
...
A Adobe Campaign Classic sempre armazena todos os MOs no banco de dados do InSMS para que essa opção não esteja disponível.
...
A Adobe Campaign Classic tem um mecanismo totalmente diferente para KPIs, portanto essa opção não está disponível.
...
Quando marcado, SMS longo será enviado em uma PDU SUBMIT_SM, colocando o texto no campo opcional message_payload. Consulte a [especificação SMPP](../../delivery/using/sms-protocol.md#ACS-SMPP-connector) para obter detalhes sobre isso.
...
Por padrão, valores de estado que começam com `DELIV`, por exemplo, `DELIVRD` no [Apêndice B](../../delivery/using/sms-protocol.md#sr-error-management), será considerado como entregue com êxito e todos os valores de estado que correspondem a erros, por exemplo, `REJECTED`, `UNDELIV`, são considerados erros.
...
>[!NOTE]
>
>Disponível apenas no conector SMPP estendido Adobe Campaign Classic.
...
>[!IMPORTANT]
>
>No Adobe Campaign Classic e em uma arquitetura híbrida, a aplicação da resposta automática para o conector SMPP estendido requer a adição de acesso de gravação para o operador mid na pasta **Conta externa**.
...
Essa configuração só funcionará se a configuração **Message payload** estiver desativada. Se a mensagem exigir mais SMS do que esse valor, um erro será acionado.
...
Ao enviar peças do delivery, o MTA gera filhos MTA. O número de processos secundários MTA é dinâmico e depende de uma configuração em serverConf.xml. Cada filho MTA instancia o conector `CSmppConnectorWorker` que se conecta ao provedor SMPP. As conexões são mantidas vivas desde que o filho MTA seja mantido vivo, também configurável em serverConf.xml.
...
Uma tabela intermediária `nmsProviderMsgId` é usada para armazenar temporariamente dados MT e SR antes de ser enviada de forma assíncrona para o catálogo.

`nmsProviderMsgId` a tabela tem três grupos de colunas:
...
Quando o MT e o SR terminarem o processamento, você deverá ter linhas completas, com informações de status e informações de catálogo.

Aqui, `iMsgId` está vinculado à tabela `nmsBroadLogMsg`, indicando o status completo/mensagem de erro.

O processo SMS verifica se há linhas completas a cada minuto e, em seguida, as processa de forma assíncrona:
...
**Conexões paralelas e de throughput**

Cada filho MTA cria uma quantidade configurável de conexões, portanto, limitar o número de filhos MTA limitará o número de conexões. Como a correlação entre os processos filhos e o tráfego do MTA está correlacionada, isso pode ser controlado, mas ainda assim ser um pouco imprevisível.
...
Verifique se não há muitos `BIND_* PDU`s. Se houver muitos deles, isso pode indicar que a conexão é instável. Consulte a seção [Problemas com conexões instáveis](../../delivery/using/sms-protocol.md#issues-unstable-connection) para obter mais informações.
...
### Verifique com seu provedor {#provider}
